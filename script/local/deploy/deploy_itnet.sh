...
SSH_SRC_FILE=./target/mod-marccat-1.2.0-SNAPSHOT.jar
...
ssh_itnet_deploy(){
  ...
  nohup echo "cd /usr/local/folio/bin ; sh stop_modcat.sh; cd ../data/; java -jar mod-marccat-1.2.0-SNAPSHOT.jar" | ssh -p ${SSH_PORT} folio@${SSH_HOST} &
}
ssh_itnet_deploy
