 ...
 * @since 1.0
 */
public class DAOAuthorityCatalog extends DAOCatalog {
	public static final Log logger =
		LogFactory.getLog(DAOAuthorityCatalog.class);
	...
	private List getAccessPointTags(AuthorityItem item)
		...
		List apfs =
			find(
				"from "
					+ "librisuite.business.cataloguing.authority.AuthorityAccessPoint"
					+ " as t "
					+ " where t.itemNumber = ?",
	...
	private List getNotes(int id) throws DataAccessException {
		...
		return find(
			"from librisuite.business.cataloguing.authority.AuthorityNote as t "
				+ "where t.itemNumber = ? ",
			new Object[] { new Integer(id)},
			new Type[] { Hibernate.INTEGER });
		
//		Query query = currentSession().createSQLQuery("SELECT t.AUT_NBR,t.AUT_NTE_NBR,t.AUT_NTE_TYP_CDE,t.AUT_NTE_LANG_SCRPT_CDE,t.AUT_NTE_STRNG_TXT from AUT_NTE {t} where nte.AUT_NBR=" + id,"nte",librisuite.business.cataloguing.authority.AuthorityNote.class);
//		
//		List result = null;
//		try {
//			result = query.list();
...
//		return result;
	...
	/* (non-Javadoc)
	 * @see librisuite.business.cataloguing.common.DAOCatalog#updateCacheTable(librisuite.business.cataloguing.common.CatalogItem)
	 */
	...
	public List getValidHeadingTypeList(String headingType, Locale locale)
		...
		return find(
			"select distinct new "
				+ "librisuite.business.codetable.ValueLabelElement(ct.code, ct.longText) "
