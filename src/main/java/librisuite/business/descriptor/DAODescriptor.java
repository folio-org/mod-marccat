...
public abstract class DAODescriptor extends HibernateUtil {
	...
	/**
	 * @deprecated Use
	 *             {@link librisuite.business.descriptor.DAOMadesDescriptor#getMadesCount(TTL_HDG_HRCY,int)}
	 *             instead
	 */
	...
	public void persist(Descriptor descriptor) throws DataAccessException {
		...
		if (descriptor.isChanged() && descriptor.changeAffectsCacheTable()) {
			...
			if (supportsMades()) {
				madesDescriptor.updateMadesCacheTable(descriptor);
			}
		} else {
			persistByStatus(descriptor);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.libricore.librisuite.common.HibernateUtil#delete(librisuite.business
	 *      .common.Persistence)
	 */
	public void delete(Persistence p) throws ReferentialIntegrityException,
		...
		if (supportsMades()) {
			try {
				if (madesDescriptor.getMadesCountIgnoringViews(d) > 0) {
					throw new ReferentialIntegrityException(
							"Mades AccessPoint", d.getClass().getName());
				}
