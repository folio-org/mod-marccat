...
public class TermExpressionNode implements ExpressionNode {
	 ...
	 */
	TermExpressionNode(final Locale locale, final UserProfile userProfile, final int searchingView) {
		this.locale = locale;
	...
	public String getValue() throws CclParserException {
		...
		try {
			...
			final String result = "select distinct "
	...
	private String prepareTerm() throws CclParserException {
		...
		try {
			...
			switch (semantic().getQueryActionCode()) {
				...
				case "TO":
					return String.format(semantic().getWhereClause(), sf, userProfile.getMainLibrary());
				case "TTO":
					return String.format(semantic().getWhereClause(), sf, sf, userProfile.getMainLibrary());
				default:
					return null;
			}
