...
public class RecordCorrelationDAO extends CorrelationDAO {
  ...
  public Correlation getRecordCorrelation(final Session session, final String tag, final char firstIndicator,
      final char secondIndicator, final int categoryCode, final String queryFrom) throws HibernateException {

    final List<Correlation> correlations = (categoryCode != 0)
        ? session.find(
            queryFrom + queryWhereTag + queryOrFistInd + queryFirstInd + queryOrSecondInd
                + "c.key.marcSecondIndicator <> '@' and " + "c.key.marcTagCategoryCode = ?",
   ...
   */
  @SuppressWarnings("unchecked")
  public List<Correlation> getCategoryCorrelation(final Session session, final String tag, final char firstIndicator,
      final char secondIndicator, final String queryFrom) throws HibernateException {
   ...
   * @return a CorrelationKey object containing the MARC encoding (tag and
   *         indicators) or null when none found.
   */
  @SuppressWarnings("unchecked")
  public CorrelationKey getMarcEncoding(final int category, final int firstCorrelation, final int secondCorrelation,
      final int thirdCorrelation, final Session session, final String queryFrom) throws HibernateException {
    final List<Correlation> result = session.find(
        queryFrom + "where c.key.marcTagCategoryCode = ? and " + "c.databaseFirstValue = ? and "
            + "c.databaseSecondValue = ? and " + "c.databaseThirdValue = ?",
  ...
  public List<CorrelationKey> getFilteredTag(final String tagNumber, String queryFrom, final Session session)
      throws HibernateException {
    final StringBuilder sqlFilter = new StringBuilder(queryFrom).append(queryWhereTag).append(queryFirstInd)
        .append(querySecondInd).append(" v.key.marcTag = c.key.marcTag and ")
