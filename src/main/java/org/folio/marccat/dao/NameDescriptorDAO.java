...
public class NameDescriptorDAO extends DescriptorDAO {
  ...
  public int getXrefCount(final Descriptor source, final int cataloguingView, final Session session)
    ...
    List<Integer> countList = session.find(
        "select count(*) from NME_NME_TTL_REF as ref " + WHERE_REF_NAME_HEADING_NUMBER + AND_REF_SOURCE_HEADING_TYPE_NH
    ...
    countList = session.find(
        "select count(*) from NME_TO_TTL_REF as ref " + WHERE_REF_NAME_HEADING_NUMBER + AND_REF_SOURCE_HEADING_TYPE_NH
            + AND_REF_USER_VIEW_STRING + View.makeSingleViewString(cataloguingView) + "'",
        new Object[] { source.getKey().getHeadingNumber() }, new Type[] { Hibernate.INTEGER });
    count = count + countList.get(0);
    return count;
  ...
  public List<REF> getCrossReferences(final Descriptor source, final int cataloguingView, final Session session)
    ...
    refList.addAll(session.find(
        "from NME_NME_TTL_REF as ref " + WHERE_REF_NAME_HEADING_NUMBER + AND_REF_SOURCE_HEADING_TYPE_NH
            + AND_REF_USER_VIEW_STRING + View.makeSingleViewString(cataloguingView) + "'",
        new Object[] { source.getKey().getHeadingNumber() }, new Type[] { Hibernate.INTEGER }));
    refList.addAll(session.find(
        "from NME_TO_TTL_REF as ref " + WHERE_REF_NAME_HEADING_NUMBER + AND_REF_SOURCE_HEADING_TYPE_NH
            + AND_REF_USER_VIEW_STRING + View.makeSingleViewString(cataloguingView) + "'",
        new Object[] { source.getKey().getHeadingNumber() }, new Type[] { Hibernate.INTEGER }));
    return refList;
  }
  ...
  public REF loadReference(final Descriptor source, final Descriptor target, final short referenceType,
    ...
    final String nameToTitle = new StringBuilder().append("from NME_TO_TTL_REF as ref ")
        .append(" where ref.nameHeadingNumber = ? AND ").append(" ref.titleHeadingNumber = ? AND ")
    ...
    final String nameNameTitle = new StringBuilder().append("from NME_NME_TTL_REF as ref ")
        .append(" where ref.nameHeadingNumber = ? AND ").append(" ref.nameTitleHeadingNumber = ? AND ")
        .append(" ref.sourceHeadingType = 'NH' AND ")
        .append(" ref.userViewString = '" + View.makeSingleViewString(cataloguingView) + "' AND ")
        .append(" ref.type = ").toString();
  ...
  public void delete(final Persistence p, final Session session) throws HibernateException {
    ...
    final List<Integer> countList = session.find(
        "select count(*) from NME_TTL_HDG as t where " + " t.nameHeadingNumber = ? and " + " t.key.userViewString = '"
            + View.makeSingleViewString(View.toIntView(nameHeading.getUserViewString())) + "'",
        new Object[] { nameHeading.getKey().getHeadingNumber() }, new Type[] { Hibernate.INTEGER });
