...
public interface CatalogingInformation {
   ...
   * <p>
   * In case of 008 field, leader will be used to get related values: if header
   * type code selected doesn't match with leader value, a default value string
   * will'be returned, based on leader value. Example: recordType (leader/05) =
   * 'a' bibliographicLevel (leader/06) = 'm' 008 --> 008 - Book
   *
  ...
  static FixedField getFixedField(final StorageService storageService, final int headerTypeCode, final String code,
    ...
    if (isFixedField(code) && checkParameters(code, headerTypeCode, leader)) {
      ...
      case Global.PHYSICAL_DESCRIPTION_TAG_CODE:
        final String categoryOfMaterial = ofNullable(Global.PHYSICAL_TYPES_MAP.get(headerTypeCode))
            .orElse(Global.UNSPECIFIED);
        fixedField.setHeaderTypeCode(
            (categoryOfMaterial.equals(Global.UNSPECIFIED)) ? Global.PHYSICAL_UNSPECIFIED_HEADER_TYPE : headerTypeCode);
        fixedField.setDescription(
            storageService.getHeadingTypeDescription(fixedField.getHeaderTypeCode(), lang, Global.INT_CATEGORY));
      ...
      case Global.DATETIME_TRANSACTION_TAG_CODE:
        fixedField.setDescription(storageService.getHeadingTypeDescription(headerTypeCode, lang, Global.INT_CATEGORY));
  ...
  static void setMaterialValues(final FixedField fixedField, final GeneralInformation gi) {
    ...
    if ("1".equals(gi.getMaterialDescription008Indicator())) {
      displayValue = (fixedField.getDisplayValue().length() != Global.MATERIAL_FIELD_LENGTH ? gi.getValueString()
          : fixedField.getDisplayValue());
    } else { // 006
      displayValue = fixedField.getDisplayValue().length() != Global.OTHER_MATERIAL_FIELD_LENGTH ? gi.getValueString()
          : fixedField.getDisplayValue();
    }
