...
public class RecordUtils {
  ...
  public static String getLeaderValue(boolean isBib) {
    if (isBib)
      return Global.FIXED_LEADER_LENGTH + Global.RECORD_STATUS_CODE + Global.RECORD_TYPE_CODE
          + Global.BIBLIOGRAPHIC_LEVEL_CODE + Global.CONTROL_TYPE_CODE + Global.CHARACTER_CODING_SCHEME_CODE
          + Global.FIXED_LEADER_BASE_ADDRESS + Global.ENCODING_LEVEL + Global.DESCRIPTIVE_CATALOGUING_CODE
          + Global.LINKED_RECORD_CODE + Global.FIXED_LEADER_PORTION;
    else
      return Global.FIXED_LEADER_LENGTH + Global.RECORD_STATUS_CODE + Global.AUT_RECORD_TYPE_CODE + Global.EMPTY_VALUE
          + Global.EMPTY_VALUE + Global.CHARACTER_CODING_SCHEME_CODE + Global.FIXED_LEADER_BASE_ADDRESS
          + Global.AUT_ENCODING_LEVEL + Global.PUNCTUATION_POLICY + Global.EMPTY_VALUE + Global.FIXED_LEADER_PORTION;
  ...
  public static Field addTagTransactionDate(final String lang, final StorageService storageService) {
    ...
    fixed005.setDescription(
        storageService.getHeadingTypeDescription(Global.DATETIME_TRANSACTION_HEADER_TYPE, lang, Global.INT_CATEGORY));
  ...
  public static boolean isMandatory(final Field field, final RecordTemplate template) {
    if (ofNullable(template).isPresent()) {
      return template.getFields().stream().filter(f -> f.getCode().equals(field.getCode()))
          .anyMatch(Field::isMandatory);
    }
  ...
  public static int getTagCategory(final Field field, final StorageService storageService) {
    ...
    return storageService.getTagCategory(field.getCode(), field.getVariableField().getInd1().charAt(0),
        field.getVariableField().getInd2().charAt(0), hasTitle);
  }
   ...
   */
  public static Field createCatalogingSourceField(final Map<String, String> configuration,
      final StorageService storageService, final String lang) {
    final CorrelationValues correlationValues = new CorrelationValues(Global.CATALOGING_SOURCE_HEADER_TYPE,
        Global.CORRELATION_UNDEFINED, Global.CORRELATION_UNDEFINED);

    final String description = storageService.getHeadingTypeDescription(Global.CATALOGING_SOURCE_HEADER_TYPE, lang,
        Global.INT_CATEGORY);
    final Validation validation = storageService.getSubfieldsByCorrelations(Global.INT_CATEGORY,
        correlationValues.getValue(1), correlationValues.getValue(2), correlationValues.getValue(3));
  ...
  public static Field createControlNumberField(final StorageService storageService, final String lang) {
    final String description = storageService.getHeadingTypeDescription(Global.CONTROL_NUMBER_HEADER_TYPE, lang,
        Global.INT_CATEGORY);
  ...
  public static Field createRequiredLeaderField(final StorageService storageService, final String lang) {
    final String description = storageService.getHeadingTypeDescription(Global.LEADER_HEADER_TYPE, lang,
        Global.INT_CATEGORY);
   ...
   */
  public static Field createRequiredMaterialDescriptionField(final Map<String, String> configuration,
      final StorageService storageService, final String lang) {

    final String description = storageService.getHeadingTypeDescription(Global.MATERIAL_DESCRIPTION_HEADER_TYPE, lang,
        Global.INT_CATEGORY);
   ...
   */
  public static VariableField getVariableField(final int categoryCode, final String ind1, final String ind2,
      final String code, final CorrelationValues correlations, final String description, final Validation validation) {
